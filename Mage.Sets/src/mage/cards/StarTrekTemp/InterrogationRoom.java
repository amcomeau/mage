 ...
 * @author amcomeau
 */
public final class InterrogationRoom extends CardImpl {

    public InterrogationRoom(UUID ownerId, CardSetInfo setInfo) {
        ...
        Ability ability = new ActivateAsSorceryActivatedAbility(Zone.BATTLEFIELD, new InterrogationRoomEffect(), new ManaCostsImpl("{3}{U}"));
        ability.addTarget(new TargetOpponent());
        this.addAbility(ability);

    }

    private InterrogationRoom(final InterrogationRoom card) {
        super(card);
    }

    @Override
    public InterrogationRoom copy() {
        return new InterrogationRoom(this);
    }
}

class InterrogationRoomEffect extends OneShotEffect {

    public InterrogationRoomEffect() {
    ...
    public InterrogationRoomEffect(final InterrogationRoomEffect effect) {
        super(effect);
    }

    @Override
    public InterrogationRoomEffect copy() {
        return new InterrogationRoomEffect(this);
    }
    ...
    public boolean apply(Game game, Ability source) {
        ...
        if (opponent != null && opponent.getHand().size() > 0) {
            ...
            if (revealedCard.isInstant()
                ...
                opponent.moveCardToExileWithInfo(revealedCard, source.getSourceId(), "Interrogation Room", source, game, Zone.HAND, true);
                AsThoughEffect effect = new InterrogationRoomCastFromExileEffect();
                effect.setTargetPointer(new FixedTarget(revealedCard.getId()));
                game.addEffect(effect, source);
                OneShotEffect effect2 = new ReturnFromExileEffect(Zone.HAND);
                Condition condition = new InterrogationRoomCondition(source.getSourceId(), revealedCard.getId());
...
class InterrogationRoomCastFromExileEffect extends AsThoughEffectImpl {

    InterrogationRoomCastFromExileEffect() {
        super(AsThoughEffectType.PLAY_FROM_NOT_OWN_HAND_ZONE, Duration.Custom, Outcome.Benefit);
        staticText = "You may cast that card without paying its mana cost as long as it remains exiled";
    }

    InterrogationRoomCastFromExileEffect(final InterrogationRoomCastFromExileEffect effect) {
        super(effect);
    }
    ...
    @Override
    public InterrogationRoomCastFromExileEffect copy() {
        return new InterrogationRoomCastFromExileEffect(this);
    }
...
class InterrogationRoomCondition implements Condition {

    protected UUID exileId;
    protected UUID cardId;

    public InterrogationRoomCondition(UUID exileId, UUID cardId) {
